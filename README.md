...
| Machine certificate, defect gap, all-or-nothing | `SGC.Bridge.MachineCertificate.machineDefect_gap`, `machine_point_tv_exact`, `linear_certificate_implies_descends`, `tmBlock_descends_shrink` | deterministic defect is `0` or in `[1, 2)`; exact terminal error `1 - Q^h(q x, q f^h x)` from a point input; the uniform certificate certifies `p < 1/2` only for exact factor maps; head-moving machine window theorem (uniform sufficiency) |
