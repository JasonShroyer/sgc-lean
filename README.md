...
| Machine certificate and window theorem | `SGC.Bridge.MachineCertificate.machine_terminal_reliability`, `tmBlock_descends_shrink` | decoding certificate on a blocked deterministic machine; deterministic quotients have Dobrushin coefficient `1`; for a head-moving machine the `b`-step block map descends onto the window of radius `r - b` |
