...
**Jason Shroyer** (SGC project), with drafting assistance. Version 0.1, 2026-09-12.
...
> Then for every `N` the Fourier-truncation defect satisfies
> `limsup_{t -> T*} ||D_N(u(t))|| = inf`; in particular no fixed finite coarse
> description is exactly lumpable on a neighbourhood of `T*`.
>
