...
theorem gap_pos_iff_ker_eq_span_one [Nontrivial V] {pi_dist : V → ℝ} (hπ : ∀ v, 0 < pi_dist v)
  ...
  · -- Reverse
    ...
    -- Quadratic form Q on Euclidean space
    ...
    have h_cont : ContinuousOn Q_euc S_euc := by
      ...
      · apply Continuous.matrix_mulVec
        -- Continuity of from_euc_std
        apply continuous_pi
        intro y
        apply Continuous.div (continuous_apply y) continuous_const (fun _ => Real.sqrt_ne_zero'.mpr (hπ y))
      · simp [from_euc_std]
        apply Continuous.div (continuous_apply x) continuous_const (fun _ => Real.sqrt_ne_zero'.mpr (hπ x))
    ...
    have h_pos_min : 0 < min_val := by
      ...
      have : inner_pi pi_dist v_min constant_vec_one = 0 := by
        ...
        simp [to_euc_std] at h_min_ortho
    ...
    · -- Nonempty
      ...
      have h_scale : inner_pi pi_dist (H *ᵥ v) v / inner_pi pi_dist v v = Q_euc u := by
        simp [Q_euc, from_euc_std, to_euc_std, u, inner_pi]
        have norm_eq : inner_pi pi_dist v v = ‖v_euc‖^2 := by
          simp [inner_pi, to_euc_std, EuclideanSpace.norm_sq_eq_sum]; apply Finset.sum_congr rfl
        ...
        rw [mul_div_cancel₀ _ (pow_ne_zero 2 (norm_ne_zero_iff.mpr (fun h => v.2.1 (by
          intro h2; funext x; specialize h2 x; simp [to_euc_std] at h2; apply h
          rwa [mul_eq_zero, Real.sqrt_eq_zero (le_of_lt (hπ x)), or_false] at h2))))]
      rw [h_scale]
      apply h_le_min
      constructor
