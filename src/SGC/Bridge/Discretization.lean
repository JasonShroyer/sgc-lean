...
/-! ### 7. Dirichlet Gap Consistency -/

/-- The **discrete Dirichlet gap** (Rayleigh quotient infimum) of a Laplacian.

    Note: For non-reversible generators, this is the coercivity constant, not the
    eigenvalue-based spectral gap. See `DirichletGap` in Lumpability.lean. -/
noncomputable def DiscreteDirichletGap (L : Matrix V V ℝ) (pi_dist : V → ℝ) : ℝ :=
  SGC.DirichletGap L pi_dist

/-- **Gap Consistency**: The discrete Dirichlet gap converges to the continuous gap.

    This is the key bridge property: discretization preserves the Dirichlet gap
    in the limit. -/
def GapConsistent (target : ContinuumTarget V) (L_seq : ℕ → Matrix V V ℝ)
    (pi_seq : ℕ → V → ℝ) : Prop :=
  Filter.Tendsto (fun n => DiscreteDirichletGap (L_seq n) (pi_seq n))
                 Filter.atTop
...
/-- **Bridge to Stability**: The discrete stability results (dirichlet_gap_non_decrease)
    are consistent with continuous stability.

    If the discrete Dirichlet gap is preserved under coarse-graining,
    and the discrete gap converges to the continuous gap (gap_consistent),
    then coarse-graining respects the continuous physics.

    This theorem references SGC.dirichlet_gap_non_decrease which shows:
    DirichletGap_bar L P π ≥ DirichletGap L π

    **Reversibility Note**: The spectral interpretation requires L to be self-adjoint. -/
theorem bridge_to_stability
    ...
    SGC.DirichletGap_bar L P pi_dist ≥ SGC.DirichletGap L pi_dist :=
  SGC.dirichlet_gap_non_decrease L P pi_dist hL hS hT_bdd
