...
**SGC Priority Claim (revised 2026-09-12)**: We propose SGC as a discrete, operator-theoretic counterpart to Miranda's continuous geometric hydrodynamics, and we conjecture a correspondence between the Universal Approximation Theorem and the h-principle for isocontact embeddings (overparameterization as embedding codimension). Both are research bridges, not theorems. What is kernel-proven: `SGC.Bridge.DiscreteFluidDynamics` (finite-state continuity, equilibrium = zero current, cycle/NESS, lift flexibility, viscous time budget `1/nu`) and `trajectory_closure_bound` (a finite-state validity-horizon bound under the model's damping assumptions). The earlier sentence "this explains why topological computation persists despite viscosity" is withdrawn: the current result establishes a finite-state persistence bound; its relationship to viscosity-resistant continuum fluid computation is a proposed bridge, not a theorem.

Status of the September 2026 blowup results (OpenAI, `openai/NavierStokesAndEuler`): a self-assessed Lean artifact claiming forced finite-time breakdown of 3D Navier-Stokes (Clay alternatives C/D) and unforced Euler blowup with BKM-budget divergence. Public source read; not locally replayed. It neither confirms nor refutes any SGC bridge lemma; it is orthogonal to the Miranda-Moore axis and a research target for the BKM/horizon axis (`SGC.Bridge.AbstractBKM`, L0 proven 2026-09-12).
